...
Під час запуску команди `npm run install:all` автоматично створюються два файли конфігурації (копіюючись із відповідних `.env.example`):
1. `.env` у корені репозиторію (використовується бекендом).
2. `frontend/.env` у папці фронтенду (використовується Vite-клієнтом).

#### Налаштування кореневого `.env` (Бекенд):
...
#### Налаштування `frontend/.env` (Фронтенд):

Для того, щоб на сторінці авторизації з'явилася кнопка входу через Google, у файлі `frontend/.env` необхідно вказати той самий клієнтський ідентифікатор Google:
```bash
